    ...
    if (config.web){
      try{
          for (var route in config.web.routes){
            ...
            var routePath = '/' + config.meshName+'/'+_this.name+'/'+route;
            ...
            if (routeTarget == 'static'){
              ...
              this.mesh.data.context.connect.use(routePath, serveStatic(_this.module.directory + '/' + route));
            }else{
              this.mesh.data.context.connect.use(routePath, _this.module.instance[routeTarget].bind(_this.module.instance));
            }
          } 
      }catch(e){
