...
function ComponentInstance() {
  ...
  _this._loadModule = function(module){
    ...
    if (_this.config.scope && _this.config.scope.depth &&  _this.config.scope.depth != 'local'){

      var scopeVariableName =  _this.config.scope.variableName?_this.config.scope.variableName:'scope';

      if (_this.module[scopeVariableName])
        throw new MeshError('Module already has a variable named ' + scopeVariableName + ', cannot set the scope variable.');

      if (_this.config.scope.depth == 'data') _this.module[scopeVariableName] = _this.mesh.api.data;
      

      if (_this.config.scope.depth == 'api') _this.module[scopeVariableName] = _this.mesh.api;
      
        
      if (_this.config.scope.depth == 'mesh') _this.module[scopeVariableName] = _this.mesh;
    ...
    _this._operate = function(method, parameters, callback){
      try{
        ...
        for(var i in methodSchema.parameters) {
          param = methodSchema.parameters[i]
          if(param.type == 'callback') callbackIndex = i
        };
        ...
        if(callbackIndex <= 0) parameters.push(callbackProxy)
        else if(parameters.length - 1 < callbackIndex) parameters.push(callbackProxy)
  ...
  _this._attach = function(config, callback){
    ...
    function(e, publication){
      ...
      _this._operate(method, message.args, function(e, responseArguments){
