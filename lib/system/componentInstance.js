...
function ComponentInstance() {
    ...
    if (!cached || !_description){
      ...
      for (var methodName in _this.module){
        ...
        if (methodName.indexOf('_') != 0 && typeof _this.module[methodName] == 'function'){
          _description.methods[methodName] = {"parameters":_this.mesh.util.getFunctionParameters(method)};
  ...
  _this._attach = function(){
    ...
    //attach module to the transport layer
     ...
     //console.log(_this.mesh.data);

    _this.mesh.data.on('/' + _this.config.name + '/*', {event_type:'all'}, function(e){
