...
function Mesh() {
  ...
  _this.initialize = function(config, callback){
    ...
    async.series([
      ...
      function(callback) { _this.api.util.log('initialized data layer'); _this._initializeModules(config.modules, callback) },

      function(callback) { _this.api.util.log('initialized components'); _this._instantiateComponents(config, callback) },

      function(callback) { _this.api.util.log('initialized description'); _this._registerDescription(config.name, callback) },
      // function(callback) { _this.api.util.log('initialized endpoints'); _this._initializeEndpoints(config, callback) },
      function(callback) { _this.api.util.log('initialized endpoints'); API._initializeEndpoints(_this, _this.describe(), freebase, config, callback) },

      // function(callback) { _this.api.util.log('initialized exchange api layer'); _this._createExchangeAPILayer(callback) },
      function(callback) { _this.api.util.log('initialized exchange api layer'); API._createExchangeAPILayer(_this, callback) },

      // function(callback) { _this.api.util.log('initialized event api layer'); _this._createEventAPILayer(callback) }
      function(callback) { _this.api.util.log('initialized event api layer'); API._createEventAPILayer(_this, callback) }

    ], function(e){
      if (!e){
        _this.api.util.log('mesh up...');
      }
      callback(e);
    });
  }
