...
function Mesh() {
  ...
  _this._attachModule = function(moduleName, moduleConfig, moduleInstance){

    if (moduleConfig.scope && moduleConfig.scope.depth &&  moduleConfig.scope.depth != 'local'){

      var scopeVariableName =  moduleConfig.scope.variableName?moduleConfig.scope.variableName:'scope';

      if (moduleInstance[scopeVariableName])
        throw new MeshError('Module already has a variable named ' + scopeVariableName + ', cannot set the scope variable.');

      if (moduleConfig.scope.depth == 'data') moduleInstance[scopeVariableName] = _this.api.data;
      

      if (moduleConfig.scope.depth == 'api') moduleInstance[scopeVariableName] = _this.api;
      
        
      if (moduleConfig.scope.depth == 'mesh') moduleInstance[scopeVariableName] = _this;
        ...
        if (moduleConfig.constructor){
          ...
          if (moduleConfig.constructor.type == 'async'){
            ...
            constructorCallBack = function(){
              ...
              for (var index in arguments){
                ...
                if (callBackParameter.parameterType == 'instance' && value){
                  ...
                  _this._attachModule(moduleName, moduleConfig, value);
           ...
           var moduleInstance;
            ...
            _this._attachModule(moduleName, moduleConfig, moduleInstance);
            eachCallback();
          }
        }else{
          _this._attachModule(moduleName, moduleConfig, moduleBase);
          eachCallback();
        }
      }, 
      function(err){
...
*/
      ...
      (function(endPoint, endpointName){
        for(var componentName in endPoint.description.components) {
          ...
          (function(componentDescription, componentName){
            ...
            (function(eventKey){
              eventAPI[endpointName][componentName] = {
                on:function(key, handler){
                  endPoint.on(eventKey + key, handler);
                },
                off:function(key){
                  endPoint.off(eventKey + key);
                }
              };
              if(endpointName == _this.config.name){
                eventAPI[endpointName][componentName]._emit = function(key, data, callback){
                  endPoint.set(eventKey + key, {noStore:true}, data, callback);
                }
              }
            })(eventKey)
  ...
  _this.stop = function(options, callback){
	...
	_this.start = function(config, callback){
