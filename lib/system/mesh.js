...
function Mesh() {
  ...
  _this._registerMessenger = function(meshDescription, endpoint){
    ...
    console.log(meshDescription);

    //create local exchange
    for (var componentName in meshDescription.components){
      var componentDescription = meshDescription.components[componentName];

      for (var methodName in componentDescription.methods){
        var methodDescription = componentDescription.methods[methodName];

        _this.exchange['/' + meshDescription.name + '/' + componentDescription.name + '/' + methodName] = new Messenger(endpoint, componentDescription, methodDescription, _this.components[componentName]);
      }
    }
